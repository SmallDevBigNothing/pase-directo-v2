 ...
 */
const madHourFormatter = new Intl.DateTimeFormat('en-US', {
  timeZone: 'Europe/Madrid',
  hour: 'numeric',
  hour12: false
});

const madFullFormatter = new Intl.DateTimeFormat('en-US', {
  timeZone: 'Europe/Madrid',
  year: 'numeric',
  month: 'numeric',
  day: 'numeric',
...
function buildIsoDatetime(timeStr) {
  ...
  const now = new Date();
  const parts = madHourFormatter.formatToParts(now);
  ...
  const utcParts = madFullFormatter.formatToParts(utcDate);
  const val = {};
